...
export function Sidebar({ className, children }: SidebarGenericProps) {
  return (
    <aside className={cn(['border-r border-border flex flex-col h-screen space-y-6', className])}>
        {children}
    </aside>
  );
}
...
export function SidebarNavHeaderTitle({ className, children }: SidebarGenericProps) {
    return (
     <h4 className={cn(['text-xs upapercase text-muted-foreground ml-3', className])}>
        {children}
      </h4>
    );
}
